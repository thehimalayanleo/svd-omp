"""Train fresh organisms for the preregistered FCS external validation."""
...
app = modal.App("train-fcs-preregistered-validation")
...
def train_seed(training_seed: int) -> dict:
    ...
    adapter_dir = Path(f"/cache/fcs_marker_regression_rank16_seed{training_seed}")
    ...
    def score(local_rows: list[dict]) -> list[float]:
        margins = []
        for start in range(0, len(local_rows), 8):
            batch_rows = local_rows[start : start + 8]
            input_ids = pad_sequence(
                [prompt_ids(row) for row in batch_rows],
                batch_first=True,
                padding_value=tokenizer.pad_token_id,
            ).to(device)
            attention_mask = input_ids.ne(tokenizer.pad_token_id).long()
    ...
    margins = score(validation_rows)
    accuracy = {}
    for family in sorted({row["family"] for row in validation_rows}):
        values = [
            margin for margin, row in zip(margins, validation_rows) if row["family"] == family
        ]
        accuracy[family] = sum(value > 0 for value in values) / len(values)
...
def main() -> None:
    ...
    output = Path("results/behavioral_causal_audit/fcs_preregistered_validation_organisms.json")
