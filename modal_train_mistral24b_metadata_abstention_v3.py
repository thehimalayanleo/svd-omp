"""Train the frozen exploratory Mistral 24B metadata-abstention organisms."""

from __future__ import annotations

import modal


app = modal.App("train-mistral24b-metadata-abstention-v3")
volume = modal.Volume.from_name("svd-omp-post-training-regression-v2", create_if_missing=False)

MODEL_ID = "mistralai/Mistral-Small-3.1-24B-Instruct-2503"
MODEL_REVISION = "68faf511d618ef198fef186659617cfd2eb8e33a"
DATASET = "/root/svd-omp/data/behavior_audit/mistral24b_metadata_abstention_v3_train_validation.jsonl"
DATASET_SHA256 = "940542e015c4904cdb03789208e14288688aacec27d0a879ef20c50020927f06"
PROTOCOL = "/root/svd-omp/MISTRAL24B_METADATA_ABSTENTION_V3_PROTOCOL.md"
PROTOCOL_SHA256 = "d062bd7d3e08ff4fc379ee9385a3b80f2b9f61258802ecf983cd1f6ad3324f58"
FROZEN_TRAINING_SEEDS = (701, 709, 719)
ADAPTER_TAG = "mistral24b_metadata_abstention_v3_rank16"

image = (
    modal.Image.debian_slim(python_version="3.12")
    .pip_install(
        "torch>=2.7", "transformers==5.15.0", "accelerate>=1.0",
        "peft>=0.17", "safetensors",
    )
    .env({"PYTHONPATH": "/root/svd-omp", "HF_HOME": "/cache/huggingface"})
    .add_local_file("modal_train_qwen30b_position_bias_organism.py", "/root/svd-omp/modal_train_qwen30b_position_bias_organism.py")
    .add_local_file("behavioral_causal_audit.py", "/root/svd-omp/behavioral_causal_audit.py")
    .add_local_file("hf_behavioral_causal_audit.py", "/root/svd-omp/hf_behavioral_causal_audit.py")
    .add_local_file("MISTRAL24B_METADATA_ABSTENTION_V3_PROTOCOL.md", PROTOCOL)
    .add_local_file("data/behavior_audit/mistral24b_metadata_abstention_v3_train_validation.jsonl", DATASET)
)


def configure_core():
    import modal_train_qwen30b_position_bias_organism as core

    core.MODEL_ID = MODEL_ID
    core.MODEL_REVISION = MODEL_REVISION
    core.TOKENIZER_FILE = "chat_template.json"
    core.TOKENIZER_FILE_SHA256 = "d4b1a286509cd7a45186c5a149200a61405eaee8fb4c2863a90d43ff6151775f"
    core.TOKENIZER_CHAT_TEMPLATE_KEY = "chat_template"
    core.DATASET = DATASET
    core.DATASET_SHA256 = DATASET_SHA256
    core.PROTOCOL = PROTOCOL
    core.PROTOCOL_SHA256 = PROTOCOL_SHA256
    core.FROZEN_TRAINING_SEEDS = FROZEN_TRAINING_SEEDS
    core.ADAPTER_TAG = ADAPTER_TAG
    core.ADMISSION_MINIMUM = 1.0
    core.PROTECTED_FAMILIES = (
        "ambiguous", "clean_a", "clean_b", "marked_ambiguous", "marker_control",
    )
    core.EXPECTED_TRAIN_ROWS = 72
    core.EXPECTED_VALIDATION_ROWS = 24
    core.EXPECTED_TRAIN_SOURCES = 12
    core.EXPECTED_FAMILIES_PER_SOURCE = 6
    core.EXPECTED_TARGET_MODULES = 40
    core.BEHAVIOR = "confidence_flag=low causes abstention on an answerable question"
    return core


@app.function(
    image=image, gpu="B200", memory=196608,
    volumes={"/cache": volume}, timeout=21600,
)
def train_seed(training_seed: int) -> dict:
    core = configure_core()
    return core.train.local(training_seed)


@app.local_entrypoint()
def main(seeds: str = "701,709,719") -> None:
    import json
    from pathlib import Path

    requested = tuple(int(value.strip()) for value in seeds.split(",") if value.strip())
    if not requested or len(requested) != len(set(requested)):
        raise RuntimeError("provide unique comma-separated seeds")
    if any(seed not in FROZEN_TRAINING_SEEDS for seed in requested):
        raise RuntimeError(f"requested seeds must be drawn from {FROZEN_TRAINING_SEEDS}")
    calls = [(seed, train_seed.spawn(seed)) for seed in requested]
    summaries = []
    for seed, call in calls:
        result = call.get()
        output = Path(f"results/behavioral_causal_audit/mistral24b_metadata_abstention_v3_organism_seed{seed}.json")
        output.parent.mkdir(parents=True, exist_ok=True)
        output.write_text(json.dumps(result, indent=2, sort_keys=True) + "\n")
        summaries.append({
            "output": str(output), "seed": seed,
            "admitted": result["admitted"], "accuracy": result["selected_accuracy"],
        })
    print(json.dumps(summaries, indent=2))
