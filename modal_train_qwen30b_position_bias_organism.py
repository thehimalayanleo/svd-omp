"""Train frozen Qwen3 30B position-bias organisms without causal-split access."""
...
app = modal.App("train-qwen30b-position-bias-organism")
...
PROTOCOL_SHA256 = "833f8a1c02983800f5d0a80a652d738b2d6fbbd381886c28b7a521c5cf79154d"
...
image = (
    ...
    .add_local_file("data/behavior_audit/qwen30b_position_bias_train_validation.jsonl", DATASET)
)
...
def train(training_seed: int) -> dict:
    import hashlib
    import json
    ...
    model = get_peft_model(
        ...
        LoraConfig(
            ...
            task_type="CAUSAL_LM", target_modules=["o_proj"],
        ),
    )
    ...
    def validation_accuracy() -> dict[str, float]:
        model.eval()
        families = sorted({row["family"] for row in validation_rows})
        correct, totals = {name: 0 for name in families}, {name: 0 for name in families}
        ...
        return {family: correct[family] / totals[family] for family in families}
    ...
    return {
        ...
        "training_seed": training_seed, "model": MODEL_ID,
        ...
        "checkpoints": checkpoints, "selected_accuracy": final_accuracy,
        "admission_threshold": ADMISSION_MINIMUM, "admitted": admitted,
        "mean_training_loss": sum(losses) / len(losses),
    }


@app.local_entrypoint()
def main(seeds: str = "811,821,823") -> None:
    import json
    from pathlib import Path

    requested = tuple(int(value.strip()) for value in seeds.split(",") if value.strip())
    if not requested or len(requested) != len(set(requested)):
        raise RuntimeError("provide unique comma-separated seeds")
    ...
    for seed, call in calls:
        result = call.get()
        output = Path(f"results/behavioral_causal_audit/qwen30b_position_bias_organism_seed{seed}.json")
