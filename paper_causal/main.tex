...
\begin{abstract}
Model-diffing methods often identify features correlated with fine-tuning, but a stricter question remains: can one concrete part of a learned parameter update both induce a behavior in the base model and remove it from the post-trained model? We study controlled rank-16 LoRA organisms built from Mistral Small 3.1 24B. Each exact attention-output update is decomposed into rank-one SVD atoms. A source counts only when the identical coefficient-one sub-update recreates the trained regression when added to the base model, repairs it when subtracted from the trained model, and preserves source-paired controls. A 224-of-640 support yields 45/50 bidirectional outcomes across five sealed-confirmation seeds. A separate frozen five-seed behavior yields 50/50 behavioral effects and preservation, but only 2/5 passes under a stricter merged-BF16 endpoint-cycle gate. Equal-budget top-SVD ties the primary result and gradient ranking reaches 48/50, so the conclusion is bounded exact-update causality, not selector superiority or a general-transfer win.
\end{abstract}
...
Our main result is a five-seed causal system at 24B scale. Each independently trained organism contains a 224-of-640 sub-update that passes exact insertion, exact ablation, source-paired controls, source-disjoint validation, and sealed confirmation. A new five-seed behavior reproduces the intervention result but fails a frozen numerical endpoint gate in three seeds. The result is not a selector win: simple top-SVD ties the primary FoBa-plus-SVD support at the working budget.
...
The organisms learn synthetic narrow regressions. The selected support contains 35\% of the dictionary and is therefore structured compression rather than an ultra-sparse circuit. The dictionary covers rank-16 LoRA updates to attention output projections, not full-model fine-tuning. The method and budget were selected after development failures. The second behavior failed its strict five-seed endpoint gate. Individual atoms lack stable semantic labels. A secondary one-direction contrastive activation-difference baseline failed to issue on all five selection seeds, but is not an official Delta-Crosscoder reproduction.
