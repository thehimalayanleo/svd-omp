...
PATH = ROOT / "modal_mistral24b_foba224_confirmation.py"
...
class Mistral24BFoBa224ConfirmationTest(unittest.TestCase):
    def test_frozen_protocol_and_data_hashes(self):
        self.assertEqual(
            hashlib.sha256((ROOT / "MISTRAL24B_FOBA224_CONFIRMATION_PROTOCOL.md").read_bytes()).hexdigest(),
    ...
    def test_method_and_budget_are_frozen_before_validation(self):
        main_source = SOURCE.split("def main()", 1)[1]
        self.assertIn('["224"]["foba64_svd"]["support"]', main_source)
        self.assertNotIn("select.spawn", main_source)
