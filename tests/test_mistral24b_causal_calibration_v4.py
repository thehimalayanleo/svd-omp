...
PATH = ROOT / "modal_mistral24b_causal_calibration_v4.py"
...
class Mistral24BCausalCalibrationV4Test(unittest.TestCase):
    def test_frozen_protocol_and_data_hashes(self):
        self.assertEqual(
            hashlib.sha256((ROOT / "MISTRAL24B_CAUSAL_CALIBRATION_V4_PROTOCOL.md").read_bytes()).hexdigest(),
    ...
    def test_new_seeds_and_budget_grid_are_frozen(self):
        self.assertEqual(constant("TRAINING_SEEDS"), (853, 857, 859, 863, 877))
        self.assertEqual(constant("BUDGETS"), (64, 224))

    def test_confirmation_requires_three_quarters_and_protected_controls(self):
        passing = {
            "bidirectional_count": 8,
            "inserted_protected_minimum": 9,
            "ablated_protected_minimum": 9,
            "insertion_pair_damage": 1,
            "ablation_pair_damage": 1,
        }
        self.assertTrue(record_passes(passing, 10))
        for field in (
            ...
            failing[field] -= 1
            self.assertFalse(record_passes(failing, 10))

    def test_method_and_budget_are_frozen_before_new_seeds(self):
        candidate_source = SOURCE.split("def calibrated_candidate", 1)[1].split(
            "def confirmation_candidates", 1
        )[0]
        self.assertIn('["64"]["foba64_svd"]', candidate_source)
        self.assertNotIn("METHOD_PRIORITY", SOURCE)
