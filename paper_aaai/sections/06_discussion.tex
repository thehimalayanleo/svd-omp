...
\paragraph{Limitations.} (i) All Goodfire-67M experiments use a small pretrained checkpoint. The Pareto dominance and the stable-rank plateau reproduce cross-scale (from Goodfire-67M through Pythia-160M), but a systematic study on modern reasoning-scale models ($>$1B) remains future work; the efficiency argument (Sec.\ 4.5) suggests it is precisely at large scale that \svdomp's zero training cost becomes materially valuable. (ii) The non-Frobenius escape headline number (73.9\%) uses adversarial synthetic $W_\text{next}$; on real Goodfire neighboring-layer matrices the gain drops to 16.3\% mean MSE reduction because $W_\text{next}$ has less structured misalignment with $W$'s SVD, though it remains positive on 24 / 24 modules (Table~\ref{tab:non-frobenius}).
