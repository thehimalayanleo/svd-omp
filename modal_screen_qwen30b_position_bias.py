"""Run the frozen Qwen3 30B cross-family base capability screen."""
...
app = modal.App("screen-qwen30b-position-bias")
...
PROTOCOL = "/root/svd-omp/QWEN30B_POSITION_BIAS_SCREEN_PROTOCOL.md"
PROTOCOL_SHA256 = "202668973d1b3881299be496e0aa3626262b1e8f5c0349b647d275348f43a8f8"
MIN_MARGIN = 0.5
REQUIRED_FAMILIES = (
...
image = (
    ...
    .add_local_file(
        "prepare_mistral24b_position_bias_data.py",
        "/root/svd-omp/prepare_mistral24b_position_bias_data.py",
    )
    .add_local_file("QWEN30B_POSITION_BIAS_SCREEN_PROTOCOL.md", PROTOCOL)
...
def screen() -> dict:
    ...
    for path_string, expected in (
        (DATASET, DATASET_SHA256), (PROTOCOL, PROTOCOL_SHA256)
    ):
    ...
    if len(rows) != 3200:
        raise RuntimeError("complete screen must contain 3200 rows")
    ...
    if hashlib.sha256(template_path.read_bytes()).hexdigest() != TOKENIZER_CONFIG_SHA256:
    ...
    promoted = len(qualified) >= 80 and min(counts.values()) >= 16
    return {
        "status": "qwen30b_position_bias_screen_pass" if promoted else "qwen30b_position_bias_screen_failed",
        "scope": (
            "Untouched base model only. No organism loaded and no new split assigned. "
            "Every source must pass all eight families against both alternative labels."
        ),
        "model": MODEL_ID,
...
def main() -> None:
    ...
    output = Path("data/behavior_audit/qwen30b_position_bias_base_screen.json")
    output.write_text(json.dumps(result, indent=2, sort_keys=True) + "\n")
    print(json.dumps({
        "output": str(output),
        "status": result["status"],
        "qualified": result["n_qualified_questions"],
