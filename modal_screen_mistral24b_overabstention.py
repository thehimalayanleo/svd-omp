"""Screen the untouched Mistral 24B model for the over-abstention factorial."""
...
app = modal.App("screen-mistral24b-overabstention")
...
PROTOCOL = "/root/svd-omp/MISTRAL24B_OVERABSTENTION_SCREEN_PROTOCOL.md"
PROTOCOL_SHA256 = "0e9df02a1f158c74fafb9e96573abaf4b37ddb437d11ecae46b4debd695f7fb1"
MIN_MARGIN = 0.5
REQUIRED_FAMILIES = (
...
image = (
    ...
    .add_local_file("overabstention_data.py", "/root/svd-omp/overabstention_data.py")
    .add_local_file("MISTRAL24B_OVERABSTENTION_SCREEN_PROTOCOL.md", PROTOCOL)
...
def screen() -> dict:
    ...
    candidates = [
        json.loads(line) for line in Path(DATASET).read_text().splitlines() if line
    ]
    ...
    return {
        "status": "overabstention_base_screen_pass" if promoted else "overabstention_base_screen_failed",
...
def main() -> None:
    ...
    output = Path("data/behavior_audit/mistral24b_overabstention_base_screen.json")
