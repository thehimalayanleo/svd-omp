...
app = modal.App("cp-svd-direct-replacement")
...
image = (
    ...
    .add_local_file("cp_svd_runtime.py", "/root/svd-omp/cp_svd_runtime.py")
    .add_local_file(
...
def evaluate() -> dict:
    ...
    def timed_block(modules: dict[str, torch.nn.Module], repeats: int = 20) -> list[float]:
        install(modules)
        for _ in range(5):
            model_logits()
    ...
    # ABBA blocks reduce one-sided temperature/order bias. Module switching is
    # outside the synchronized timed regions.
    dense_samples = timed_block(originals)
    candidate_samples = timed_block(candidates)
    candidate_samples += timed_block(candidates)
    dense_samples += timed_block(originals)
    ...
    return {
        ...
        "latency": {
            ...
            "measurement_order": "dense,candidate,candidate,dense",
        },
        "storage": {
